...
console.log(salesByWeek[0]["sales_agent"]["first_name"], salesByWeek[0]["sales_agent"]["last_name"]);
...
salesByWeek.forEach(sale => {
    container.innerHTML += `<hr/><h2>${sale["sales_agent"]["first_name"]} ${sale["sales_agent"]["last_name"]}</h2>`

        for (const entry of Object.entries(sale.vehicle)) {
            container.innerHTML += `<div>${entry[0]}: ${entry[1]}</div>`
        }
        container.innerHTML += `<h3>Profit: $${sale["gross_profit"]}</h3>`
});